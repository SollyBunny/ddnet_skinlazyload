...
void CMapSounds::OnRender()
{
	...
	for(auto &Source : m_vSourceQueue)
	{
		...
		{
			s_Time = mix((Client()->PrevGameTick(g_Config.m_ClDummy) - m_pClient->m_Snap.m_pGameInfoObj->m_RoundStartTick) / (float)g_Config.m_SvTickRate,
				(Client()->GameTick(g_Config.m_ClDummy) - m_pClient->m_Snap.m_pGameInfoObj->m_RoundStartTick) / (float)g_Config.m_SvTickRate,
